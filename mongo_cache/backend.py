...
class MongoDBCache(BaseCache):
	def __init__(self, location, params):
		...
		self._rs_hosts = options.get('RS_HOSTS')
		self._rs_name = options.get('RS_NAME')
		self._collection = location

	def make_key(self, key, version=None):
		"""
		Additional regexp to remove $ and . cachaters, as they cause special behaviour in mongodb
		"""
	...
	def _base_set(self, mode, key, value, timeout=None):
		...
		try:
			if data and (mode == 'set' or
				...
				coll.update({'_id': data['_id']}, {'$set': {'data': encoded, 'expires': expires}}, safe=True)
			else:
	...
	def _initialize_collection(self):
		if self._rs_hosts and self._rs_name:
			self.connection = pymongo.MongoClient(self._rs_hosts, replicaSet=self._rs_name)
		else:
